...
module.exports = {
    ...
    /* Busca en BBDD*/
    select: function (table, obj, order, limit, offset, count, mid_functions, callback) {

        mid_functions = (mid_functions) ? mid_functions : [];

        connection.select(table, obj, order, limit, offset, count, function (error, result) {
            if (error) {
