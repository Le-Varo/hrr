 ...
 * 
 */
function search(query, type, parameters, callback) {
    if (types[type]) {
        types[type](query, parameters, function (error, result) {
            if (error) {
                callback(error, null);
            } else {
...
function miscellaneousFunctions(type, funct, parameters, callback) {
    if (types[type] && types[type][funct]) {
        types[type][funct](parameters, callback);
    } else {
        callback(new Error(METH_NOTFOUND), null);
    }
}
...
module.exports = {
    ...
    search: search,
    miscellaneousFunctions: miscellaneousFunctions
}