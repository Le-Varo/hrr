...
function difference(source, comparator) {
    var toReturn = {};
...
function update(table, obj1, obj2, schema, callback) {
    select(table, obj1, schema, /*null,null,null,null */ function (err, result) {
        if (err) {
            callback(err, null);
        } else if (result[0]) {
            var Model = proxyModelBySchema(table, schema);
            ...
            db.open(function () {
                result.forEach(function (item) {
                    var obj = difference(item, obj2);
                    Model.updateOne(item, obj).then(function (updated) {
