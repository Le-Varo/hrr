...
function combineSecondPreference(source, comparator) {
    var toReturn = Object.assign(source);
    Object.keys(comparator).forEach(function (item, key) {
        if (item != comparator[key]) {
            toReturn[key] = comparator[key];
        }
    });
    return toReturn;
}

function update(table, query, obj, schema, callback) {
    select(table, query, schema, /*null,null,null,null */ function (err, result) {
        if (err) {
            ...
            db.open(function () {
                result.forEach(function (item) {
                    Model.updateOne(item, obj).then(function () {
                        toReturn.push(combineSecondPreference(item, obj));
                        counter -= 1;
                        if (counter == 0) {
                            db.close();
                            callback(null, toReturn);
