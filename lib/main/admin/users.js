...
function modify(user1, user2, callback) {
    db.update("users", user1, user2, userSchema, function (error, result) {
        if (error) {
            callback(error, null);
        } else {
...
function validate(access_token, callback) {
    var query = {
    ...
    _get(query, function (error, result) {
        if (error) {
            ...
            modify(result[0], user, function (error) {
                if (error) {
                    callback(error);
                } else {
                    callback(null);
