 ...
 *    - access_token:   Access_Token del usuario a activar
 ...
 * 
 */
function activate(tokenID, access_token, callback) {
    // Primero activamos el token (eliminamos de BBDD)
    registerToken.activate(tokenID, access_token, function (error, result) {
        if (error) {
            callback(error, null);
        } else {
...
function activateByAccessToken(users, count, callback) {
    if (typeof users === "string") {
        users = [users];
    }
    ...
    activate(null, users[count], function (error) {
        if (error) {
            callback(error);
        } else {
            count += 1;
            if (count === users.length) {
                ...
                activateByAccessToken(users, count, callback);
            }
        }
    });
}
...
module.exports = {
    ...
    remove: remove,
    activateByAccessToken: activateByAccessToken
}