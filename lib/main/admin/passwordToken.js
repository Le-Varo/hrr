...
function sendToken(token, user, callback) {
    ...
    link = "http://" + host + "/reset?id=" + token + "&newPassword=" + newPassword;
    mailOptions = {
        to: user.email,
        subject: "Reset your Password",
        generateTextFromHTML: true,
        html: "Hello,<br> Please Click on the link to reset your password.<br><a href=" + link + ">Click here to activate</a>"
    }
...
function generate(user, callback) {
    ...
    db.insert("passwordToken", token, tokenSchema, function (error, token) {
        if (error) {
            callback(error, null);
        } else {
