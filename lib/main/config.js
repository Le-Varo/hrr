...
nconf.argv().env("__"); // Primero tiene en cuenta los argumentos y luego las variables del entorno.
...
nconf.defaults({ // Estos son los valores por defecto, si alguno de estos campos no esta incluido en config.json
    ...
    'registerToken': {
        'TTL': 86400000 // Milisegundos en los que el token está activo
    },
    'resetToken': {
        'TTL': 3600000 // Milisegundos en los que el token está activo
    },
    'lang': 'EN-en',
});
...
module.exports = {
    ...
    getResetTokenConfig: function () {
        return nconf.get('resetToken'); // Devuelve la configuración relativa a los token de registro
    },
    getUsersConfig: function () {
        return nconf.get('users'); // Devuelve la configuración relativa a los usuarios
    },
    getComicTypeConfig: function () {
        return nconf.get("comicType"); // Devuelve la configuración relativa al tipo de item comics
    },
    getVideoTypeConfig: function () {
        return nconf.get("videoType"); // Devuelve la configuración relativa al tipo de item videojuegos
    },
